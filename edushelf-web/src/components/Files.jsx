...
const Files = () => {
  ...
  const fetchFiles = async () => {
    try {
      const data = (!searchTerm && !searchTag)
        ? await getDocuments(currentPage, pageSize)
        : await searchDocuments(searchTerm, searchTag, currentPage, pageSize);
      setFiles(data.items);
  ...
  useEffect(() => {
    fetchFiles();
  }, [currentPage, pageSize, searchTerm, searchTag]);
  ...
  const toggleMenu = (fileId) => setOpenMenuId(openMenuId === fileId ? null : fileId);

  const filteredFiles = files.filter(file => {
    if (rejectedFiles.includes(file.id)) return false;
    if (!file.isShared) return true;
    return acceptedFiles.includes(file.id);
  });
  ...
  if (viewFile) {
    return <FileViewer file={viewFile} onClose={() => setViewFile(null)} />;
  }
      ...
      <div className="file-list">
              ...
              <div className="file-card-buttons">
                ...
                {openMenuId === file.id && (
                  <div className="dropdown-menu">
                    <button onClick={() => setViewFile(file)} title="View"><FaEye /> View</button>
                    {!file.isShared && (
                      <button onClick={() => { setFileToEdit(file); setIsEditOpen(true); setOpenMenuId(null); }} title="Edit">
                        <FaEdit /> Edit
                      </button>
                    )}
                    {!file.isShared && (
        ...
        {isUploadDialogOpen && (
          <UploadDialog
            ...
            onUploadSuccess={() => { setUploadDialogOpen(false); fetchFiles(); }}
          />
        )}
